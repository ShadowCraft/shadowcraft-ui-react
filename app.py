...
def character_show(region, realm, name):
    data = ShcUrlHandler.get_character_data(mongo, region, realm, name)
    return "data"
...
def character_refresh(region, realm, name):
    ShcUrlHandler.refresh_character(mongo, region, realm, name)
...
def history_getsha(character_data):
    return ShcUrlHandler.get_debug_sha(mongo, character_data)
...
def get_items_by_slot(slot, min_ilvl=-1, max_ilvl=-1):
    return ShcUrlHandler.get_items_by_slot(mongo, slot, min_ilvl, max_ilvl)
