"""this module contains the model for enchant data in mongo"""
import pymongo

def init_db(dbase):
    """define index"""
    dbase.enchants.create_index([('spell_id', pymongo.ASCENDING)], unique=True)


def populate_db(dbase):
    """some hardcoded mappings between spellid and enchant id
    this mapping is not available via any known api generally this
    is taken from simc's database when the mapping has already been done"""
