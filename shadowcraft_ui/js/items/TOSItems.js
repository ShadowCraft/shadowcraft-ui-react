...
const baseTOSItems = TOS_ITEMS.map(id => {
    const item = ITEM_DATA.find(item => item.id === id);
    if (item !== undefined) return item;
    else console.log(`Item ${id} defined in TOSItems could not be found.`);
});
const filteredItems = (slot) => baseTOSItems.filter(item => item !== undefined && item.equip_location === slot);
const mapItemsToBonuses = (slot, ilvl) => filteredItems(slot).map(item => createItem(item, ilvl, TOS_BONUSES[ilvl]));
