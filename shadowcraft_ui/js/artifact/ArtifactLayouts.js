export const kingslayers_layout = {
    artifact: 'kingslayers',
    artifact_name: 'The Kingslayers',
    artifact_icon: 'inv_knife_1h_artifactgarona_d_01',
    primary_trait: 192759,
    paragon_trait: 214928,
    second_major: 241152,
    concordance: 239042,
    relics: ['Shadow', 'Iron', 'Blood'],
    traits: {
        'ks_toxicblades': {
            name: 'Toxic Blades',
            icon: 'ability_rogue_disembowel',
            x: 258,
            y: 266,
            id: 192310,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'ks_serratededge': {
            name: 'Serrated Edge',
            icon: 'ability_warrior_bloodbath',
            x: 437,
            y: 288,
            id: 192315,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'ks_masteralchemist': {
            name: 'Master Alchemist',
            icon: 'trade_brewpoison',
            x: 47,
            y: 344,
            id: 192318,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'ks_fadeintoshadows': {
            name: 'Fade into Shadows ',
            icon: 'spell_shadow_nethercloak',
            x: 128,
            y: 455,
            id: 192323,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'ks_balancedblades': {
            name: 'Balanced Blades',
            icon: 'ability_rogue_restlessblades',
            x: 264,
            y: 365,
            id: 192326,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'ks_gushingwound': {
            name: 'Gushing Wound',
            icon: 'ability_rogue_bloodsplatter',
            x: 34,
            y: 458,
            id: 192329,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'ks_shadowwalker': {
            name: 'Shadow Walker',
            icon: 'ability_rogue_sprint',
            x: 150,
            y: 276,
            id: 192345,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'ks_masterassassin': {
            name: 'Master Assassin',
            icon: 'ability_rogue_deadliness',
            x: 139,
            y: 348,
            id: 192349,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'ks_poisonknives': {
            name: 'Poison Knives',
            icon: 'ability_rogue_dualweild',
            x: 341,
            y: 376,
            id: 192376,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'ks_urgetokill': {
            name: 'Urge to Kill',
            icon: 'ability_rogue_improvedrecuperate',
            x: 205,
            y: 282,
            id: 192384,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'ks_shadowswift': {
            name: 'Shadow Swiftness',
            icon: 'rogue_burstofspeed',
            x: 190,
            y: 384,
            id: 192422,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'ks_surgeoftoxins': {
            name: 'Surge of Toxins',
            icon: 'ability_rogue_deviouspoisons',
            x: 380,
            y: 324,
            id: 192424,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'ks_fromtheshadows': {
            name: 'From the Shadows',
            icon: 'ability_rogue_deadlybrew',
            x: 449,
            y: 196,
            id: 192428,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'ks_bagoftricks': {
            name: 'Bag of Tricks',
            icon: 'rogue_paralytic_poison',
            x: 94,
            y: 255,
            id: 192657,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'ks_kingsbane': {
            name: 'Kingsbane',
            icon: 'inv_knife_1h_artifactgarona_d_01',
            x: 364,
            y: 215,
            id: 192759,
            max_rank: 1,
            ring: 'thick',
            relic: false
        },
        'ks_bota': {
            name: 'Blood of the Assassinated',
            icon: 'inv_artifact_bloodoftheassassinated',
            x: 94,
            y: 552,
            id: 192923,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'ks_assassinsblades': {
            name: 'Assassin\'s Blades',
            icon: 'ability_rogue_shadowstrikes',
            x: 308,
            y: 243,
            id: 214368,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'ks_slayersprecision': {
            name: 'Slayer\'s Precision',
            icon: 'inv_knife_1h_artifactgarona_d_02dual',
            x: 526,
            y: 164,
            id: 214928,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'ks_strangler': {
            name: 'Strangler',
            icon: 'ability_rogue_garrote',
            x: 590,
            y: 571,
            id: 238066,
            max_rank: 4,
            ring: 'thin',
            relic: true
        },
        'ks_denseconcoction': {
            name: 'Dense Concoction',
            icon: 'ability_rogue_crimsonvial',
            x: 515,
            y: 497,
            id: 238102,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'ks_sinistercirculation': {
            name: 'Sinister Circulation',
            icon: 'inv_knife_1h_artifactgarona_d_01dual',
            x: 593,
            y: 425,
            id: 238138,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'ks_concordance': {
            name: 'Concordance of the Legionfall',
            icon: 'trade_archaeology_shark-jaws',
            x: 593,
            y: 498,
            id: 239042,
            max_rank: 50,
            ring: 'dragon',
            relic: false
        },
        'ks_silence': {
            name: 'Silence of the Uncrowned',
            icon: 'misc_legionfall_rogue',
            x: 667,
            y: 496,
            id: 241152,
            max_rank: 1,
            ring: 'thick',
            relic: false
        }
    },
    lines: [
        {
            trait1: 'ks_toxicblades',
            trait2: 'ks_urgetokill'
        },
        {
            trait1: 'ks_toxicblades',
            trait2: 'ks_balancedblades'
        },
        {
            trait1: 'ks_toxicblades',
            trait2: 'ks_assassinsblades'
        },
        {
            trait1: 'ks_serratededge',
            trait2: 'ks_fromtheshadows'
        },
        {
            trait1: 'ks_serratededge',
            trait2: 'ks_surgeoftoxins'
        },
        {
            trait1: 'ks_masteralchemist',
            trait2: 'ks_bagoftricks'
        },
        {
            trait1: 'ks_masteralchemist',
            trait2: 'ks_masterassassin'
        },
        {
            trait1: 'ks_masteralchemist',
            trait2: 'ks_gushingwound'
        },
        {
            trait1: 'ks_fadeintoshadows',
            trait2: 'ks_bota'
        },
        {
            trait1: 'ks_fadeintoshadows',
            trait2: 'ks_shadowswift'
        },
        {
            trait1: 'ks_balancedblades',
            trait2: 'ks_shadowswift'
        },
        {
            trait1: 'ks_balancedblades',
            trait2: 'ks_poisonknives'
        },
        {
            trait1: 'ks_gushingwound',
            trait2: 'ks_masterassassin'
        },
        {
            trait1: 'ks_gushingwound',
            trait2: 'ks_bota'
        },
        {
            trait1: 'ks_shadowwalker',
            trait2: 'ks_bagoftricks'
        },
        {
            trait1: 'ks_shadowwalker',
            trait2: 'ks_urgetokill'
        },
        {
            trait1: 'ks_masterassassin',
            trait2: 'ks_shadowswift'
        },
        {
            trait1: 'ks_masterassassin',
            trait2: 'ks_urgetokill'
        },
        {
            trait1: 'ks_poisonknives',
            trait2: 'ks_surgeoftoxins'
        },
        {
            trait1: 'ks_poisonknives',
            trait2: 'ks_assassinsblades'
        },
        {
            trait1: 'ks_kingsbane',
            trait2: 'ks_assassinsblades'
        },
        {
            trait1: 'ks_strangler',
            trait2: 'ks_denseconcoction'
        },
        {
            trait1: 'ks_strangler',
            trait2: 'ks_silence'
        },
        {
            trait1: 'ks_denseconcoction',
            trait2: 'ks_sinistercirculation'
        },
        {
            trait1: 'ks_sinistercirculation',
            trait2: 'ks_concordance'
        }
    ]
};

export const kingslayers_ranking = [
    {
        name: '',
        items: [
            {
                name: 'Toxic Blades',
                id: 192310
            },
            {
                name: 'Serrated Edge',
                id: 192315
            },
            {
                name: 'Master Alchemist',
                id: 192318
            },
            {
                name: 'Fade into Shadows ',
                id: 192323
            },
            {
                name: 'Balanced Blades',
                id: 192326
            },
            {
                name: 'Gushing Wound',
                id: 192329
            },
            {
                name: 'Shadow Walker',
                id: 192345
            },
            {
                name: 'Master Assassin',
                id: 192349
            },
            {
                name: 'Poison Knives',
                id: 192376
            },
            {
                name: 'Urge to Kill',
                id: 192384
            },
            {
                name: 'Shadow Swiftness',
                id: 192422
            },
            {
                name: 'Surge of Toxins',
                id: 192424
            },
            {
                name: 'From the Shadows',
                id: 192428
            },
            {
                name: 'Bag of Tricks',
                id: 192657
            },
            {
                name: 'Kingsbane',
                id: 192759
            },
            {
                name: 'Blood of the Assassinated',
                id: 192923
            },
            {
                name: 'Assassin\'s Blades',
                id: 214368
            },
            {
                name: 'Slayer\'s Precision',
                id: 214928
            },
            {
                name: 'Strangler',
                id: 238066
            },
            {
                name: 'Dense Concoction',
                id: 238102
            },
            {
                name: 'Sinister Circulation',
                id: 238138
            },
            {
                name: 'Concordance of the Legionfall',
                id: 239042
            },
            {
                name: 'Silence of the Uncrowned',
                id: 241152
            }
        ]
    }
];

export const dreadblades_layout = {
    artifact: 'dreadblades',
    artifact_name: 'The Dreadblades',
    artifact_icon: 'inv_sword_1h_artifactskywall_d_01',
    primary_trait: 202665,
    paragon_trait: 214929,
    second_major: 241153,
    concordance: 239042,
    relics: ['Blood', 'Iron', 'Storm'],
    traits: {
        'db_curse': {
            name: 'Curse of the Dreadblades',
            icon: 'inv_sword_1h_artifactskywall_d_01dual',
            x: 325,
            y: 337,
            id: 202665,
            max_rank: 1,
            ring: 'thick',
            relic: false
        },
        'db_blunderbuss': {
            name: 'Blunderbuss',
            icon: 'inv_weapon_rifle_01',
            x: 534,
            y: 183,
            id: 202897,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'db_greed': {
            name: 'Greed',
            icon: 'warrior_skullbanner',
            x: 63,
            y: 574,
            id: 202820,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'db_blurredtime': {
            name: 'Blurred Time',
            icon: 'ability_rogue_quickrecovery',
            x: 541,
            y: 277,
            id: 202769,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'db_deception': {
            name: 'Deception',
            icon: 'ability_rogue_disguise',
            x: 394,
            y: 197,
            id: 202755,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'db_hiddenblade': {
            name: 'Hidden Blade',
            icon: 'ability_ironmaidens_bladerush',
            x: 142,
            y: 549,
            id: 202753,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'db_blademaster': {
            name: 'Blademaster',
            icon: 'ability_warrior_challange',
            x: 459,
            y: 312,
            id: 202628,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'db_blackpowder': {
            name: 'Black Powder',
            icon: 'inv_weapon_rifle_01',
            x: 506,
            y: 384,
            id: 216230,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'db_bladedancer': {
            name: 'Blade Dancer',
            icon: 'ability_warrior_bladestorm',
            x: 282,
            y: 443,
            id: 202507,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'db_fatesthirst': {
            name: 'Fate\'s Thirst',
            icon: 'ability_rogue_waylay',
            x: 248,
            y: 313,
            id: 202514,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'db_fortunesstrike': {
            name: 'Fortune\'s Strike',
            icon: 'spell_rogue_deathfromabove',
            x: 472,
            y: 235,
            id: 202521,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'db_gunslinger': {
            name: 'Gunslinger',
            icon: 'inv_weapon_rifle_07',
            x: 318,
            y: 213,
            id: 202522,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'db_fatebringer': {
            name: 'Fatebringer',
            icon: 'ability_rogue_cuttothechase',
            x: 141,
            y: 400,
            id: 202524,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'db_fortunestrikes': {
            name: 'Fortune Strikes',
            icon: 'ability_rogue_improvedrecuperate',
            x: 383,
            y: 383,
            id: 202530,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'db_ghostlyshell': {
            name: 'Ghostly Shell',
            icon: 'spell_shadow_nethercloak',
            x: 59,
            y: 450,
            id: 202533,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'db_fortunesboon': {
            name: 'Fortune\'s Boon',
            icon: 'ability_rogue_surpriseattack2',
            x: 383,
            y: 290,
            id: 202907,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'db_cursededges': {
            name: 'Cursed Edges',
            icon: 'inv_sword_33',
            x: 216,
            y: 412,
            id: 202463,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'db_cursedsteel': {
            name: 'Cursed Steel',
            icon: 'inv_sword_1h_artifactskywall_d_02dual',
            x: 110,
            y: 199,
            id: 214929,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'db_sabermetrics': {
            name: 'Sabermetrics',
            icon: 'ability_rogue_sabreslash',
            x: 591,
            y: 572,
            id: 238067,
            max_rank: 4,
            ring: 'thin',
            relic: true
        },
        'db_dreadbladesvigor': {
            name: 'Dreadblade\'s Vigor',
            icon: 'inv_sword_1h_artifactskywall_d_01dual',
            x: 516,
            y: 498,
            id: 238103,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'db_loadeddice': {
            name: 'Loaded Dice',
            icon: 'ability_rogue_rollthebones',
            x: 594,
            y: 425,
            id: 238139,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'db_concordance': {
            name: 'Concordance of the Legionfall',
            icon: 'trade_archaeology_shark-jaws',
            x: 594,
            y: 499,
            id: 239042,
            max_rank: 50,
            ring: 'dragon',
            relic: false
        },
        'db_silence': {
            name: 'Bravado of the Uncrowned',
            icon: 'misc_legionfall_rogue',
            x: 668,
            y: 497,
            id: 241153,
            max_rank: 1,
            ring: 'thick',
            relic: false
        }
    },
    lines: [
        {
            trait1: 'db_curse',
            trait2: 'db_cursededges'
        },
        {
            trait1: 'db_blunderbuss',
            trait2: 'db_fortunesstrike'
        },
        {
            trait1: 'db_greed',
            trait2: 'db_ghostlyshell'
        },
        {
            trait1: 'db_blurredtime',
            trait2: 'db_blackpowder'
        },
        {
            trait1: 'db_deception',
            trait2: 'db_fortunesstrike'
        },
        {
            trait1: 'db_deception',
            trait2: 'db_gunslinger'
        },
        {
            trait1: 'db_deception',
            trait2: 'db_fortunesboon'
        },
        {
            trait1: 'db_hiddenblade',
            trait2: 'db_fatebringer'
        },
        {
            trait1: 'db_hiddenblade',
            trait2: 'db_bladedancer'
        },
        {
            trait1: 'db_blademaster',
            trait2: 'db_blackpowder'
        },
        {
            trait1: 'db_blademaster',
            trait2: 'db_fortunesboon'
        },
        {
            trait1: 'db_blackpowder',
            trait2: 'db_fortunestrikes'
        },
        {
            trait1: 'db_bladedancer',
            trait2: 'db_fortunestrikes'
        },
        {
            trait1: 'db_bladedancer',
            trait2: 'db_cursededges'
        },
        {
            trait1: 'db_fatesthirst',
            trait2: 'db_gunslinger'
        },
        {
            trait1: 'db_fatesthirst',
            trait2: 'db_fatebringer'
        },
        {
            trait1: 'db_fatesthirst',
            trait2: 'db_cursededges'
        },
        {
            trait1: 'db_fatebringer',
            trait2: 'db_ghostlyshell'
        },
        {
            trait1: 'db_sabermetrics',
            trait2: 'db_dreadbladesvigor'
        },
        {
            trait1: 'db_sabermetrics',
            trait2: 'db_silence'
        },
        {
            trait1: 'db_dreadbladesvigor',
            trait2: 'db_loadeddice'
        },
        {
            trait1: 'db_loadeddice',
            trait2: 'db_concordance'
        }
    ]
};

export const dreadblades_ranking = [
    {
        name: '',
        items: [
            {
                name: 'Curse of the Dreadblades',
                id: 202665
            },
            {
                name: 'Blunderbuss',
                id: 202897
            },
            {
                name: 'Greed',
                id: 202820
            },
            {
                name: 'Blurred Time',
                id: 202769
            },
            {
                name: 'Deception',
                id: 202755
            },
            {
                name: 'Hidden Blade',
                id: 202753
            },
            {
                name: 'Blademaster',
                id: 202628
            },
            {
                name: 'Black Powder',
                id: 216230
            },
            {
                name: 'Blade Dancer',
                id: 202507
            },
            {
                name: 'Fate\'s Thirst',
                id: 202514
            },
            {
                name: 'Fortune\'s Strike',
                id: 202521
            },
            {
                name: 'Gunslinger',
                id: 202522
            },
            {
                name: 'Fatebringer',
                id: 202524
            },
            {
                name: 'Fortune Strikes',
                id: 202530
            },
            {
                name: 'Ghostly Shell',
                id: 202533
            },
            {
                name: 'Fortune\'s Boon',
                id: 202907
            },
            {
                name: 'Cursed Edges',
                id: 202463
            },
            {
                name: 'Cursed Steel',
                id: 214929
            },
            {
                name: 'Sabermetrics',
                id: 238067
            },
            {
                name: 'Dreadblade\'s Vigor',
                id: 238103
            },
            {
                name: 'Loaded Dice',
                id: 238139
            },
            {
                name: 'Concordance of the Legionfall',
                id: 239042
            },
            {
                name: 'Bravado of the Uncrowned',
                id: 241153
            }
        ]
    }
];

export const fangs_layout = {
    artifact: 'fangs',
    artifact_name: 'Fangs of the Devourer',
    artifact_icon: 'inv_knife_1h_artifactfangs_d_01',
    primary_trait: 209782,
    paragon_trait: 214930,
    second_major: 241154,
    concordance: 239042,
    relics: ['Fel', 'Shadow', 'Fel'],
    traits: {
        'fangs_goremawsbite': {
            name: 'Goremaw\'s Bite',
            icon: 'inv_knife_1h_artifactfangs_d_01',
            x: 528,
            y: 134,
            id: 209782,
            max_rank: 1,
            ring: 'thick',
            relic: false
        },
        'fangs_quietknife': {
            name: 'The Quiet Knife',
            icon: 'ability_backstab',
            x: 402,
            y: 371,
            id: 197231,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'fangs_demonskiss': {
            name: 'Demon\'s Kiss',
            icon: 'ability_priest_voidentropy',
            x: 234,
            y: 566,
            id: 197233,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'fangs_gutripper': {
            name: 'Gutripper',
            icon: 'ability_rogue_eviscerate',
            x: 464,
            y: 189,
            id: 197234,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'fangs_precision': {
            name: 'Precision Strike',
            icon: 'ability_rogue_shadowstrike',
            x: 341,
            y: 533,
            id: 197235,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'fangs_fortunesbite': {
            name: 'Fortune\'s Bite',
            icon: 'ability_rogue_masterofsubtlety',
            x: 499,
            y: 361,
            id: 197369,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'fangs_soulshadows': {
            name: 'Soul Shadows',
            icon: 'inv_knife_1h_grimbatolraid_d_03',
            x: 369,
            y: 260,
            id: 197386,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'fangs_energetic': {
            name: 'Energetic Stabbing',
            icon: 'inv_knife_1h_pvppandarias3_c_02',
            x: 220,
            y: 464,
            id: 197239,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'fangs_catlike': {
            name: 'Catlike Reflexes',
            icon: 'inv_pet_cats_calicocat',
            x: 357,
            y: 334,
            id: 197241,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'fangs_ghostarmor': {
            name: 'Ghost Armor',
            icon: 'achievement_halloween_ghost_01',
            x: 290,
            y: 405,
            id: 197244,
            max_rank: 3,
            ring: 'thin',
            relic: true
        },
        'fangs_second': {
            name: 'Second Shuriken',
            icon: 'inv_throwingknife_07',
            x: 278,
            y: 496,
            id: 197610,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'fangs_flickering': {
            name: 'Flickering Shadows',
            icon: 'ability_rogue_sprint_blue',
            x: 336,
            y: 445,
            id: 197256,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'fangs_embrace': {
            name: 'Embrace of Darkness',
            icon: 'ability_stealth',
            x: 427,
            y: 438,
            id: 197604,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'fangs_akaarissoul': {
            name: 'Akaari\'s Soul',
            icon: 'ability_warlock_soullink',
            x: 475,
            y: 283,
            id: 209835,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'fangs_shadownova': {
            name: 'Shadow Nova',
            icon: 'spell_fire_twilightnova',
            x: 382,
            y: 185,
            id: 209781,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'fangs_finality': {
            name: 'Finality',
            icon: 'ability_rogue_eviscerate',
            x: 139,
            y: 525,
            id: 197406,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'fangs_shadowfangs': {
            name: 'Shadow Fangs',
            icon: 'inv_misc_blacksaberonfang',
            x: 560,
            y: 264,
            id: 221856,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'fangs_legionblade': {
            name: 'Legionblade',
            icon: 'inv_knife_1h_artifactfangs_d_02dual',
            x: 193,
            y: 338,
            id: 214930,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'fangs_etchedinshadow': {
            name: 'Etched in Shadow',
            icon: 'ability_criticalstrike',
            x: 590,
            y: 574,
            id: 238068,
            max_rank: 4,
            ring: 'thin',
            relic: true
        },
        'fangs_shadowswhisper': {
            name: 'Shadow\'s Whisper',
            icon: 'ability_rogue_masterofsubtlety',
            x: 515,
            y: 500,
            id: 242707,
            max_rank: 1,
            ring: 'thin',
            relic: true
        },
        'fangs_feedingfrenzy': {
            name: 'Feeding Frenzy',
            icon: 'inv_knife_1h_artifactfangs_d_01',
            x: 593,
            y: 427,
            id: 238140,
            max_rank: 1,
            ring: 'dragon',
            relic: false
        },
        'fangs_concordance': {
            name: 'Concordance of the Legionfall',
            icon: 'trade_archaeology_shark-jaws',
            x: 593,
            y: 501,
            id: 239042,
            max_rank: 50,
            ring: 'dragon',
            relic: false
        },
        'fangs_shadows': {
            name: 'Shadows of the Uncrowned',
            icon: 'misc_legionfall_rogue',
            x: 667,
            y: 499,
            id: 241154,
            max_rank: 1,
            ring: 'thick',
            relic: false
        }
    },
    lines: [
        {
            trait1: 'fangs_goremawsbite',
            trait2: 'fangs_shadowfangs'
        },
        {
            trait1: 'fangs_quietknife',
            trait2: 'fangs_akaarissoul'
        },
        {
            trait1: 'fangs_quietknife',
            trait2: 'fangs_flickering'
        },
        {
            trait1: 'fangs_demonskiss',
            trait2: 'fangs_finality'
        },
        {
            trait1: 'fangs_demonskiss',
            trait2: 'fangs_precision'
        },
        {
            trait1: 'fangs_gutripper',
            trait2: 'fangs_catlike'
        },
        {
            trait1: 'fangs_gutripper',
            trait2: 'fangs_shadowfangs'
        },
        {
            trait1: 'fangs_precision',
            trait2: 'fangs_second'
        },
        {
            trait1: 'fangs_precision',
            trait2: 'fangs_embrace'
        },
        {
            trait1: 'fangs_fortunesbite',
            trait2: 'fangs_embrace'
        },
        {
            trait1: 'fangs_fortunesbite',
            trait2: 'fangs_shadowfangs'
        },
        {
            trait1: 'fangs_soulshadows',
            trait2: 'fangs_shadownova'
        },
        {
            trait1: 'fangs_soulshadows',
            trait2: 'fangs_catlike'
        },
        {
            trait1: 'fangs_energetic',
            trait2: 'fangs_finality'
        },
        {
            trait1: 'fangs_energetic',
            trait2: 'fangs_second'
        },
        {
            trait1: 'fangs_energetic',
            trait2: 'fangs_ghostarmor'
        },
        {
            trait1: 'fangs_catlike',
            trait2: 'fangs_ghostarmor'
        },
        {
            trait1: 'fangs_ghostarmor',
            trait2: 'fangs_flickering'
        },
        {
            trait1: 'fangs_etchedinshadow',
            trait2: 'fangs_shadowswhisper'
        },
        {
            trait1: 'fangs_etchedinshadow',
            trait2: 'fangs_shadows'
        },
        {
            trait1: 'fangs_shadowswhisper',
            trait2: 'fangs_feedingfrenzy'
        },
        {
            trait1: 'fangs_feedingfrenzy',
            trait2: 'fangs_concordance'
        }
    ]
};

export const fangs_ranking = [
    {
        name: '',
        items: [
            {
                name: 'Goremaw\'s Bite',
                id: 209782
            },
            {
                name: 'The Quiet Knife',
                id: 197231
            },
            {
                name: 'Demon\'s Kiss',
                id: 197233
            },
            {
                name: 'Gutripper',
                id: 197234
            },
            {
                name: 'Precision Strike',
                id: 197235
            },
            {
                name: 'Fortune\'s Bite',
                id: 197369
            },
            {
                name: 'Soul Shadows',
                id: 197386
            },
            {
                name: 'Energetic Stabbing',
                id: 197239
            },
            {
                name: 'Catlike Reflexes',
                id: 197241
            },
            {
                name: 'Ghost Armor',
                id: 197244
            },
            {
                name: 'Second Shuriken',
                id: 197610
            },
            {
                name: 'Flickering Shadows',
                id: 197256
            },
            {
                name: 'Embrace of Darkness',
                id: 197604
            },
            {
                name: 'Akaari\'s Soul',
                id: 209835
            },
            {
                name: 'Shadow Nova',
                id: 209781
            },
            {
                name: 'Finality',
                id: 197406
            },
            {
                name: 'Shadow Fangs',
                id: 221856
            },
            {
                name: 'Legionblade',
                id: 214930
            },
            {
                name: 'Etched in Shadow',
                id: 238068
            },
            {
                name: 'Shadow\'s Whisper',
                id: 242707
            },
            {
                name: 'Feeding Frenzy',
                id: 238140
            },
            {
                name: 'Concordance of the Legionfall',
                id: 239042
            },
            {
                name: 'Shadows of the Uncrowned',
                id: 241154
            }
        ]
    }
];

export const netherlight_traits = [
    {
        "name": "Master of Shadows",
        "side": "Void",
        "id": 252901
    },
    {
        "name": "Murderous Intent",
        "side": "Void",
        "id": 252191
    },
    {
        "name": "Shadowbind",
        "side": "Void",
        "id": 252875
    },
    {
        "name": "Chaotic Darkness",
        "side": "Void",
        "id": 252888
    },
    {
        "name": "Torment the Weak",
        "side": "Void",
        "id": 252906
    },
    {
        "name": "Dark Sorrows",
        "side": "Void",
        "id": 252922
    },
    {
        "name": "Light Speed",
        "side": "Light",
        "id": 252088
    },
    {
        "name": "Refractive Shell",
        "side": "Light",
        "id": 252207
    },
    {
        "name": "Shocklight",
        "side": "Light",
        "id": 252799
    },
    {
        "name": "Secure in the Light",
        "side": "Light",
        "id": 253070
    },
    {
        "name": "Infusion of Light",
        "side": "Light",
        "id": 253093
    },
    {
        "name": "Light's Embrace",
        "side": "Light",
        "id": 253111
    }
];

